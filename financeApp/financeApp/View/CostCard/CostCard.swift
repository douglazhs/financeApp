        ...
        ZStack{
            ...
            HStack{
                ...
                VStack(alignment: .leading, spacing: 20){
                    SpentDetails(user: $user, spent: spent)
                        .environmentObject(viewModel)
                    
                    SpentInfo(spent: spent)
                }
            }
        }
        .overlay(content: {
...
struct SpentDetails: View{
    @EnvironmentObject var viewModel: CostCardViewModel
    @Binding var user: User?
    var spent: Spent
    ...
    var body: some View{
        HStack{
            ...
            Text(String(format: "%.1f%", viewModel.calcultatePercentage(with: user?.budget ?? 0.0, and: spent.cost)))
                .font(.custom(URBANIST_SEMIBOLD, size: 10))
    ...
    var body: some View{
        HStack{
            ...
            Text(String(format: "R$%.2f", spent.cost))
                .font(.custom(URBANIST_BOLD, size: 14))
                .padding(.horizontal, 20)
